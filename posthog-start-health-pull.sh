...
for attempt in 1 2 3; do
    ...
    if sudo -E docker-compose -f docker-compose.new.yml up -d --no-build --pull always; then
...
if [ -z "$SKIP_HEALTH_CHECK" ]; then
    ...
    if timeout 600 bash -c 'while [[ "$(curl -s -o /dev/null -w ''%{http_code}'' localhost:8000/_health)" != "200" ]]; do sleep 5; done'; then
        HEALTH_END=$(date +%s)
